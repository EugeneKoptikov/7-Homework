...
// Finding the sum of the elements of the columns
void ArithmeticMeanSumOfColumnElement(int[,] collection)
{
    int i = 1; // number of column
    for (int jndex = 0; jndex < collection.GetLength(1); jndex++)
    {
        double sum = 0;
        ...
        for (int index = 0; index < collection.GetLength(0); index++)
        {
            sum = sum + collection[index, jndex];
        }

        sum = sum / collection.GetLength(1);
        Console.WriteLine("The sum of " + i + " column elements is: " + sum);
        i++;
    }
}
...
PrintArray(array);
ArithmeticMeanSumOfColumnElement(array);